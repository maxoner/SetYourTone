...
namespace SetYourTone.Services
{
    public class Coordinator
    {
        ...
        public Coordinator (RuleModel userRule, Dictionary<string, char> Triggers, string curUserFrame)
        {
            ...
            string curLayerExpanded = expander(userRule.startWorkPiece, leftStep, rightStep, userXLeft, userXRight,
                                               CurRule.leftBorder.Length, arrDeep, ref layerCenter);

            ArrWorkers Base = new ArrWorkers(curLayerExpanded, userRule.leftBorder, userRule.rightBorder, arrDeep);
            Base.Filler(CurRule, userRule.leftBorder.Length, userRule.rightBorder.Length);

            int xLeft  = layerCenter + userXLeft ;
            int xRight = layerCenter + userXRight;

            frame = Base.Framer(xLeft, Convert.ToInt32(stringsCurFrame[1]), xRight, Convert.ToInt32(stringsCurFrame[3]));
        }
